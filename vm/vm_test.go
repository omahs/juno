...
func TestExecute(t *testing.T) {
	...
	t.Run("empty transaction list", func(t *testing.T) {
		...
		_, _, err := New(nil).Execute([]core.Transaction{}, []core.Class{}, 0, timestamp, address, state, &network, []*felt.Felt{}, false, false, false, &felt.Zero, &felt.Zero)
		require.NoError(t, err)
	})
	t.Run("zero data", func(t *testing.T) {
		_, _, err := New(nil).Execute(nil, nil, 0, 0, &felt.Zero, state, &network, []*felt.Felt{}, false, false, false, &felt.Zero, &felt.Zero)
		require.NoError(t, err)
	})
}
