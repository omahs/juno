...
type Client struct {
	...
	writeURL string
	client   *http.Client
	// timeout is used for GET requests. POST requests will use a multiple of timeout.
...
func (c *Client) do(req *http.Request, timeout time.Duration) (*http.Response, error) {
	...
	// Shallow copy the client so we can overwrite the timeout without affecting other goroutines.
	...
	*client = *c.client
	...
	c.listener.OnResponse(req.URL.Path, resp.StatusCode, time.Since(startTime))
...
func (c *Client) post(ctx context.Context, queryURL string, data any) ([]byte, error) {
	...
	resp, err := c.do(req, 10*c.timeout) //nolint:gomnd
	if err != nil {
		return nil, err
	}
